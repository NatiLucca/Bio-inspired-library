...
PSO::PSO(int population,double boundMin,double boundMax,double C1,double C2,int iterations, double inertia, int dim, string tf){
    ...
    this->c1 = C1; /* coeficinete de aceleracao*/
    this->c2 = C2;
...
void PSO::run(double (*costFunction)(vector<double> )){
     ...
     this->global_best = costFunction(this->particles[0].position);
      ...
      for (int i=0; i<steps; i++){ /* repete tantas iterações */
             for(int j=0; j<population; j++){
                    cost_Particle = costFunction(particles[j].position); /* fitness */
...
/* abre arquivo para gravar valores usados no pso */
  ofstream archive;
  string name = "Inputs/valuesPSO" + nameArq+ ".txt";
  archive.open(name);
  /* gera valores randomicos para os coeficientes */
  random_device rd;
  mt19937 gen(rd());
  ...
  initParticles(); /*Inicializa todas as partivulas aleatoriamente*/
}
